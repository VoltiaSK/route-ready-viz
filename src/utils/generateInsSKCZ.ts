...
// Generate unique vehicle ID (license plate)
...
export const generateInsSKCZData = (): VehicleDataResponse => {
  ...
  const nonEvReadyCount = totalVehicles - evReadyCount; // 110 vehicles not EV-ready
  ...
  // EV Ready vehicles (handle 92% of routes) - daily routes between 60km and 240km
  ...
  for (let i = 0; i < nonEvReadyCount; i++) {
    ...
    if (i % 5 === 0) {
      ...
      max95perc = Math.floor(Math.max(400, avgDistance * 3 + Math.random() * 200)); // Large gap as specified
    } else {
  ...
  // Verify the EV readiness counts and ensure exactly 150 vehicles
...
if (require.main === module) {
  try {
    ...
    const jsonData = generateInsSKCZData();
    ...
    const outputPath = path.join(publicDir, 'insSKCZ.json');
...
export default generateInsSKCZData;
